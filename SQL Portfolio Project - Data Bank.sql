...
Where year(cn.End_date) <> 9999
),
 percentiles as (
 Select Region_id,Region_name,Reallodays,
percent_rank()  over (Partition by cn.region_id order by RealloDays) as percentile_rank
from reallocationdays)
Select Region_id,Region_name,
(Select Reallodays from percentiles Where percentile_rank>=0.5 and region_name=p.region_name Order by Reallodays limit 1) as median,
(Select Reallodays from percentiles Where percentile_rank>=0.8 and region_name=p.region_name Order by Reallodays limit 1) as Percentile_80,
(Select Reallodays from percentiles Where percentile_rank>=0.95 and region_name=p.region_name Order by Reallodays limit 1) as Percentile_95
From percentiles p
...
Select txn_type,round(avg(dpamount)) Avg_Total_Dp_Amount,round(avg(dpcount)) Avg_Total_Dp_Count
from Totaldp
Where txn_type = 'deposit'
group by txn_type;
...
(PurchaseCount>=1 or WithdrawalCount>=1))
group by Tmonth
order by TMonth;
...
(
Select Customer_id,
Month(txn_date) as TMonth,
Sum(Case When txn_type='Deposit' Then txn_amount
Else -txn_amount
End) as UpdatedAmount
from customer_transactions
Group by customer_id,TMonth
)
...
sum(UpdatedAmount) as totalbalance
from EachMonth
Group by Customer_id,Tmonth
),
ClosingBalance as(
Select Customer_id,tmonth,
sum(totalbalance) over (Partition by Customer_id order by Tmonth )  as CBalance
from MonthlyTotal
),
...
Select Customer_id,tmonth,CBalance,PreBalance,
Case When PreBalance is null or PreBalance=0 Then null
Else (Cbalance-PreBalance)/PreBalance*100 End as percentagegrowth
from PreClosingBalance
)
Select count(distinct Pi.Customer_id)/Count(Em.Customer_id)*100 as  Customer_Percentage
From PercentageIncrease PI
join EachMonth Em on em.Customer_id=Pi.Customer_id
where percentagegrowth>=5
;
...
#--Common Table Expression (CTE) to calculate the total balance for each customer in each transaction month
 ...
 With TotalBalance as (Select Customer_id,txn_date,
 month(Txn_date) TransactionMonth,
...
Sum(case when TotalBalance>0 then TotalBalance else 0 end ) RunningBalance
 From MonthlyBalance
 Group by  TransactionMonth
 order by TransactionMonth;
